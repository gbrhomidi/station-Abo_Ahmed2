 ...
 * - هذا العامل (Worker) لا يعتمد على الخادم المحلي (NanoHTTPD)
 *   بأي شكل من الأشكال.
 * - يستخدم DatabaseHelper مباشرة لتصدير البيانات.
 * - جميع العمليات تتم محلياً وبشكل مستقل عن طبقة الاتصال أو الواجهات.
 * - متوافق تماماً مع المعمارية الجديدة (AndroidInterface +
 *   main.html + screens/*.html).
 * ═══════════════════════════════════════════════════════════════
 */
class BackupWorker(
    context: Context,
