...
class GeminiAIHelper(private val context: Context) {
    ...
    fun initialize(apiKey: String) {
        try {
            ...
            val dangerousContentSafety = SafetySetting(HarmCategory.DANGEROUS_CONTENT, BlockThreshold.ONLY_HIGH)

            generativeModel = GenerativeModel(
                modelName = "gemini-1.5-flash",
                apiKey = apiKey,
                safetySettings = listOf(harassmentSafety, hateSpeechSafety, dangerousContentSafety)
            )
    ...
    suspend fun sendMessage(message: String): String {
        return withContext(Dispatchers.IO) {
            try {
                val model = generativeModel ?: return@withContext "خطأ: نموذج الذكاء الاصطناعي غير مهيأ. يرجى التحقق من مفتاح API."

                val stationContext = """
                    أنت مساعد ذكي متخصص لمحطة وقود (ديزل) تسمى "محطة أبو أحمد".
                    تساعد المستخدم في:
                ...
                val text = response.text ?: "لم أتمكن من فهم طلبك. هل يمكنك توضيح المزيد؟"
                ...
                "عذراً، حدث خطأ في الاتصال بالمساعد الذكي: ${e.message}"
            }
        }
    }
    ...
    fun sendMessageSync(message: String): String {
        return try {
            "يرجى استخدام sendToAI() للحصول على استجابة فورية من المساعد الذكي."
        } catch (e: Exception) {
