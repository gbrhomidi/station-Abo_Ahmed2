...
class SmsAiResourceOrchestrator(context: Context) {
    ...
    fun getHealthStatus(): JSONObject {
        ...
        val allProfiles = profiles?.list().orEmpty()
        var availableCount = 0
        var totalCooldowns = 0

        for (profile in allProfiles) {
            val isCooldown = profile.failureCount > 3 && profile.successCount == 0L // مبسط للـ Circuit Breaker
            if (!isCooldown && profile.enabled) availableCount++
            ...
            arr.put(JSONObject().apply {
                ...
                put("health_score", healthScore(profile))
                put("failures", profile.failureCount)
                put("successes", profile.successCount)
            })
        }
