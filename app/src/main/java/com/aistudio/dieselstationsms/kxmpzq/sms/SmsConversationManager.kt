...
class SmsConversationManager(private val db: DatabaseHelper) {
    ...
    private fun loadDurableDraft(phone: String): OrderDraft? {
        ...
        val json = runCatching { JSONObject(jsonText) }
            .onFailure { error -> Log.e(TAG, "Failed to decode durable draft for phone=${maskPhoneForLog(phone)}", error) }
    ...
    private fun persistDurableDraft(phone: String, draft: OrderDraft) {
        try {
            ...
            val json = runCatching { JSONObject(current) }
                .onFailure { error -> Log.e(TAG, "Failed to decode existing draft JSON for phone=${maskPhoneForLog(phone)}", error) }
                .getOrElse { JSONObject() }
            draft.version += 1L
            ...
            if (updated == 0) {
                ...
                if (inserted == -1L) {
                    Log.e(TAG, "Durable draft UPSERT failed for phone=${maskPhoneForLog(phone)}")
                }
            }
        } catch (error: Exception) {
            Log.e(TAG, "Durable draft persistence failed for phone=${maskPhoneForLog(phone)}", error)
        }
    }
