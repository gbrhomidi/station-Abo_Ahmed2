...
class SmsOperationalNervousSystem(private val db: DatabaseHelper) {
    ...
    fun recordProcessingRecovery(
        ...
        val safeMetadata = JSONObject(metadata.toString()).apply {
            put("phone_masked", phone.take(3) + "***" + phone.takeLast(3))
            put("reason", reason.take(500))
        }
        db.writableDatabase.insertWithOnConflict(
            "sms_recovery_actions", null, ContentValues().apply {
                put("recovery_id", recoveryId)
                put("target_type", "SMS_PROCESSING")
                put("target_id", targetId.take(160))
                put("policy_version", "SMS_RECOVERY_V3")
                put("action", "REVIEW_AND_RETRY_SAFELY")
                put("status", "PLANNED")
                put("reason", safeMetadata.toString().take(1500))
