 ...
 *   party_contacts.phone / phone2
 *
 * Compatibility fallback:
 *   parties.phone / phone2, only when those columns exist.
 *
 * This class deliberately keeps SMS customer lookup independent from the
 * legacy parties schema so a V7-style schema cannot break inbound SMS.
 */
class SmsPartyGateway(private val db: DatabaseHelper) {
    ...
    fun find(phone: String?): PartyMatch? {
        ...
        // Canonical lookup: party_contacts.
                ...
                COALESCE(
                    (SELECT pa.address_line1
                      ...
                      WHERE pa.party_id = p.id
                        AND pa.is_deleted = 0
                      ORDER BY pa.is_default DESC, pa.id DESC
                      LIMIT 1), ''
                ) AS address,
        ...
        // Compatibility fallback for databases that already have legacy phone columns.
        if (columnExists("parties", "phone")) {
            val sql = if (columnExists("parties", "phone2")) {
                """
            ...
            val args = if (columnExists("parties", "phone2")) arrayOf(p, p) else arrayOf(p)
            return database.rawQuery(sql, args).use { c ->
                if (c.moveToFirst()) {
                    PartyMatch(
    ...
    fun getPartyId(phone: String?): Int? = find(phone)?.partyId

    fun getBalance(phone: String?): Double = find(phone)?.balance ?: 0.0
