...
assert.match(
  screen,
  /apiCall\('getActivityLogs',\s*buildActivityQueryParams\(\)\)/,
  'screen load must use the database-backed activity log API'
);
assert.match(
  screen,
  /type:\s*\(currentFilter\s*===\s*'all'\s*\|\|\s*currentFilter\s*===\s*'ai_suggest'\)\s*\?\s*''\s*:\s*currentFilter/,
  'UI-only filters must not be sent as unsupported SQLite log types'
);
