...
function installBridge(window) {
  window.AndroidInterface = {
    openVehicleFilePicker: kind => { calls.push(['openVehicleFilePicker', kind]); return json(1); },
...
(async () => {
  ...
  window.document.getElementById('vehicle_photo').click();
  assert.ok(calls.some(c => c[0] === 'openVehicleFilePicker' && c[1] === 'photo'), 'vehicle photo field must open native file picker');
  window.onVehicleFileSelected('photo', 'content://media/external/images/101', 'hilux.jpg', 'image/jpeg');
  assert.strictEqual(window.document.getElementById('vehicle_photo').value, 'content://media/external/images/101', 'photo URI must be stored in field');
  assert.strictEqual(window.document.getElementById('vehicle_photo').dataset.fileName, 'hilux.jpg', 'photo file name must be retained');
  assert.ok(!window.document.getElementById('vehiclePhotoPreview').hidden, 'vehicle photo preview must be visible');
  window.document.getElementById('registration_doc').click();
  assert.ok(calls.some(c => c[0] === 'openVehicleFilePicker' && c[1] === 'registration'), 'registration document field must open native file picker');
  window.onVehicleFileSelected('registration', 'content://com.android.providers.downloads.documents/document/42', 'registration.pdf', 'application/pdf');
  assert.strictEqual(window.document.getElementById('registration_doc').value, 'content://com.android.providers.downloads.documents/document/42', 'registration URI must be stored in field');
  assert.strictEqual(window.document.getElementById('registration_doc').dataset.fileName, 'registration.pdf', 'registration file name must be retained');
  window.document.getElementById('color').click();
  window.document.getElementById('colorPicker').value = '#ffffff';
  window.document.getElementById('colorPicker').dispatchEvent(new window.Event('change', { bubbles: true }));
  assert.strictEqual(window.document.getElementById('color').value, 'أبيض', 'color picker must save Arabic color name');
